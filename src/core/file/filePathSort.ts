...
function compareSpecialChars(a: string, b: string): number {
...
function compareCaseSensitive(a: string, b: string): number {
  const caseOrder = (str: string) => {
    if (str === str.toUpperCase()) return 0;
    if (str === str.charAt(0).toUpperCase() + str.slice(1).toLowerCase()) return 1;
    return 2;
  };
  ...
  if (aCaseOrder !== bCaseOrder) return aCaseOrder - bCaseOrder;
...
function comparePaths(a: string, b: string): number {
  ...
  for (let i = 0; i < Math.min(aParts.length, bParts.length); i++) {
    if (aParts[i] !== bParts[i]) {
      ...
      if (specialCharCmp !== 0) return specialCharCmp;
...
function compareTestPaths(a: string, b: string): number {
  ...
  if (aIsTest && bIsTest) {
    ...
    if (aRootTest && !bRootTest) return -1;
    if (!aRootTest && bRootTest) return 1;
...
function comparePriorityFiles(a: string, b: string): number {
...
export function sortPaths(paths: string[]): string[] {
  ...
  const matchesTestCase = (input: string[]) => {
  ...
  const sorted = normalized.sort((a, b) => {
    ...
    if (!aIsDir && !bIsDir) {
      ...
      if (aNestedIndex !== -1) return -1;
      if (bNestedIndex !== -1) return 1;
    }
