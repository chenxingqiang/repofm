...
export async function configureAIProviders() {
  ...
  try {
    ...
    if (isValid) {
      ...
      if (setDefault) {
        aiProviderConfig.setDefaultProvider(providerChoice as string);
        p.note(
          `${providerChoice} is now the default AI provider`, 
          'Default Provider'
        );
      }
    } else {
...
export async function removeAIProvider() {
  ...
  delete updatedConfig.providers[providerToRemove as keyof typeof config.providers];
  ...
  p.outro(
    chalk.green(`${providerToRemove} provider removed successfully`)
  );
}
