...
export class ProjectContextAnalyzer {
     ...
     */
    static async analyzeProjectContext(projectRoot = process.cwd()) {
        try {
            ...
            const commitStyleDetectors = [
                {
                    ...
                    detector: (msg) => /^(feat|fix|docs|style|refactor|test|chore)(\(.+\))?:/.test(msg)
                },
                {
                    style: 'Angular Commits',
                    detector: (msg) => /^(build|ci|docs|feat|fix|perf|refactor|style|test)(\(.+\))?:/.test(msg)
                }
            ];
            ...
            try {
                const gitLogOutput = await new Promise((resolve, reject) => {
                    ...
                    exec('git log --pretty=format:"%s" -n 10', { cwd: projectRoot }, (err, stdout) => err ? reject(err) : resolve(stdout));
                });
                const commitMessages = gitLogOutput.split('\n');
                context.commitStyles = commitStyleDetectors
                    .filter(detector => commitMessages.some(msg => detector.detector(msg)))
