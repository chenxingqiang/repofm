...
export class CommitMessageLearner {
    ...
    async collectTrainingData() {
      ...
      `;
            ...
            return commitEntries.map(entry => {
                ...
                return {
                    fileChanges: changes.filter(change => change.startsWith('M') ||
                        change.startsWith('A') ||
                        change.startsWith('D')),
    ...
    preprocessData(data) {
        ...
        return {
            tokens: tf.tensor2d(data.map(entry => entry.fileChanges.map(change => change.charCodeAt(0))), [data.length, 50]),
            labels: tf.oneHot(tf.tensor1d(data.map(entry => entry.commitMessage.length), 'int32'), 10000)
        };
    }
    /**
     * Generate commit message using trained model
     */
    async generateCommitMessage(fileChanges) {
        if (!this.model) {
            await this.loadModel();
        }
        try {
            // Predict commit message based on file changes
            const prediction = this.model?.predict(tf.tensor2d(fileChanges.map(change => change.charCodeAt(0)), [1, 50]));
    ...
    async loadModel() {
        try {
            this.model = await tf.loadLayersModel(`file://${this.trainingDataPath}/model.json`);
        }
        catch (error) {
