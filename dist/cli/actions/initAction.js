...
export const runInitAction = async (rootDir = process.cwd(), isGlobal = false) => {
    try {
...
export async function createConfigFile(rootDir, isGlobal) {
    ...
    try {
        ...
        // 写入配置文件
        await fs.writeFile(configPath, JSON.stringify(defaultConfiguration, null, 2), 'utf-8');
...
export async function createIgnoreFile(rootDir, isGlobal) {
    ...
    try {
        ...
        // 写入忽略文件
        await fs.writeFile(ignorePath, defaultIgnorePatterns.join('\n'), 'utf-8');
