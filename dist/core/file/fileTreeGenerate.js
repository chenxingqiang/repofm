...
export function generateTreeString(files) {
    ...
    const specialFiles = files.filter(file => specialRootOrder.includes(path.basename(file)));
    // Remove special files from the main list
    const otherFiles = files.filter(file => !specialRootOrder.includes(path.basename(file)));
